	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(courierrestService.insertCourierrest(courierid, restid, userorderid)){
			...
			if(orderdetailService.updateOrderstatus(userorderid, 3) && courierService.updateCourierstatus("忙碌", courierid)){
				out.println("<script>alert('配送成功!');location='OrderQueryServlet?currentPage=1';</script>");
			}else{
				out.println("<script>alert('配送失败!');location='OrderQueryServlet?currentPage=1';</script>");
			}
		}else{
			out.println("<script>alert('Insert Error!');location='OrderQueryServlet?currentPage=1';</script>");
		}
