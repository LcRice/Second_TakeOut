...
public class AutoSelectCourierServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (orderlist.size() == 0) {
			out.println("<script>alert('暂无订单!');location='GreensQueryServlet?currentPage=1';</script>");
		}
		...
		for (Orderdetail o : orderlist) {
			...
			if (courier != null) {
				if (courierrestService.insertCourierrest(courier.getCourierid(), restid, o.getUserorderid())) {
					if (orderdetailService.updateOrderstatus(o.getUserorderid(), 3)
							&& courierService.updateCourierstatus("忙碌", courier.getCourierid())) {
						out.println("<script>alert('配送成功!');location='GreensQueryServlet?currentPage=1';</script>");
					} else {
						out.println("<script>alert('配送失败!');location='GreensQueryServlet?currentPage=1';</script>");
					}
				} else {
					out.println("<script>alert('Insert Error!');location='GreensQueryServlet?currentPage=1';</script>");
				}
			} else {
				out.println("<script>alert('无空闲配送员！请稍后再试！');location='GreensQueryServlet?currentPage=1';</script>");
			}
		}
	}
