...
public class OrderdetailDAOImpl implements OrderdetailDAO {
	...
	public List<Orderdetail> findOrderdetailListByUser(int currentPage, int pageSize, int userid) {
		// TODO Auto-generated method stub
		String sql = "SELECT courier.couriername,greens.greensname,restaurant.restname,orderdetail.orderstatus,orderdetail.userorderid,ordersummary.ordertime"
				+ " from orderdetail JOIN ordersummary ON orderdetail.orderid = ordersummary.orderid and userid=?"
				+ " JOIN greens ON greens.greensid = orderdetail.greensid"
				+ " JOIN restaurant ON greens.restid = restaurant.restid"
				+ " LEFT JOIN courierrest ON courierrest.userorderid = orderdetail.userorderid"
				+ " LEFT JOIN courier ON courier.courierid = courierrest.courierid limit ?, ?";
		...
		try {
			while (rs.next()) {
				...
				ordersummary.setOrdertime(rs.getTimestamp(6));
	...
	public int findTotalCount(int userid) {
		...
		String sql = "SELECT count(greens.greensname)"
				...
				+ " JOIN restaurant ON greens.restid = restaurant.restid"
				+ " LEFT JOIN courierrest ON courierrest.userorderid = orderdetail.userorderid"
				+ " LEFT JOIN courier ON courier.courierid = courierrest.courierid";
	...
	public List<Orderdetail> findFinishedOrderListByUser(int currentPage, int pageSize, int userid) {
		...
		String sql = "SELECT tttt.couriername,tttt.greensname,restaurant.restname,tttt.orderstatus,tttt.userorderid from restaurant JOIN"
				...
				+ " (select courier.couriername,courierrest.userorderid" + " from courierrest JOIN courier"
				...
				+ " on greens.greensid = ttt.greensid) as tttt" + " on restaurant.restid = tttt.restid limit ?,?";
	...
	public List<Orderdetail> findOrderdetailListByRest(int currentPage, int pageSize, int restid) {
		...
		String sql = "select `user`.username,greens.greensname,t.count,t.orderstatus,ordersummary.ordertime from ordersummary JOIN"
				...
				+ " from orderdetail" + " where orderdetail.greensid in"
				+ " (select greensid from greens where restid = ?)) as t" + " on ordersummary.orderid = t.orderid"
	...
	public int findRestTotalCount(int restid) {
		...
		String sql = "select count(`user`.username) from ordersummary JOIN"
				...
				+ " from orderdetail" + " where orderdetail.greensid in"
				+ " (select greensid from greens where restid = ?)) as t" + " on ordersummary.orderid = t.orderid"
				+ " JOIN user on ordersummary.userid=`user`.userid" + " JOIN greens on t.greensid = greens.greensid";
	...
	public List<Orderdetail> findUnfinishedOrderListByRest(int restid) {
		...
		ResultSet rs = dbManager.execQuery(sql, restid, 5);
	...
	public int getMaxUserOrderid() {
		// TODO Auto-generated method stub
		String sql = "select max(userorderid) from orderdetail";
	...
	public List<Orderdetail> findOrderListByOrderid(int orderid) {
		// TODO Auto-generated method stub
		String sql = "select * from orderdetail where orderid = ?";

		ResultSet rs = dbManager.execQuery(sql, orderid);
